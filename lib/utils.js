...
const getModuleRoot = _.memoize(function getModuleRoot () {
  ...
  while (!isAtFsRoot) {
    ...
    try {
      if (_fs.existsSync(manifestPath) &&
          JSON.parse(_fs.readFileSync(manifestPath, 'utf8')).name === '@limrun/appium-webdriveragent') {
        return currentDir;
      }
    } catch {}
  ...
  throw new Error('Cannot find the root folder of the @limrun/appium-webdriveragent Node.js module');
});
